...
## 2. 会议软件配置（Zoom / Teams / 飞书 / 腾讯会议 / 企业微信 / Discord / Meet 均可）

判据只有一个：**该软件能否手动指定麦克风/扬声器设备**。能就行，与具体软件无关。

- **麦克风** = `BlackHole 2ch`（接收 CLI 注入的译文）
- **扬声器** = `BlackHole 16ch`（CLI 从这里采集对方原声）

各软件设置位置：
- **飞书**：设置 → 音频（或会议中点麦克风旁 ∧）→ 选麦克风/扬声器
- **腾讯会议 / 企业微信**：设置 → 音频
- **Zoom**：Settings → Audio
- **Teams**：设置 → 设备

**通用注意（务必关闭）：**
- 关闭"**自动调整麦克风音量 / AGC**"，避免译文音量忽大忽小。
- 关闭"**智能降噪 / 语音增强 / 啸叫抑制**"。⚠️ 飞书等默认对麦克风输入做 AI 降噪——而这里的"麦克风"是 BlackHole 注入的**合成译文音频**，降噪可能把它当噪声削弱甚至吃掉。飞书在 **设置 → 音频** 关掉降噪类选项。
